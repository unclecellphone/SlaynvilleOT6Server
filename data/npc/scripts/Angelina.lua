...
if msgcontains(msg, 'teleport') and npcHandler.focus == cid then
	...
	npcHandler:say("I am still gathering my strength for a teleport home, but some power already has returned. Do you wish to be teleported out of this cell?", cid)
	talk_state = 9166
...
elseif talk_state == 0 and msgcontains(msg, 'job') and npcHandler.focus == cid then
	npcHandler:say("I am a priestess and I travelled here to learn about that order of the humble path I heard about. ...", cid)
	npcHandler:say("But when I started my investigations, this false monk Lorbas thought that I was suspicious and so he ordered his minions to take me as prisoner.", cid)

elseif talk_state == 0 and msgcontains(msg, 'cult') and npcHandler.focus == cid then
	npcHandler:say("The cult is secretly looking for the unsatisfied, disgrunteled and poor. Its members promise such sad individuals wealth, revenge and a cause. ...", cid)
...
elseif talk_state == 0 and msgcontains(msg, 'cathedral') and npcHandler.focus == cid then
	npcHandler:say("The cathedral was meant to be a centre of piety and believe. A prayer to the gods that had become solid. ...", cid)
