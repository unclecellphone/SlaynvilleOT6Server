...
function creatureSayCallback(cid, type, msg)
	...
	npcHandler:say("Sorry but I won\'t talk about this matter with someone of your rank.", cid)
	topic = 0
	
	elseif msgcontains(msg, 'special operations') and getPlayerStorageValue(cid,STORAGE_POSTMAN_DOOR) > 3 then
	npcHandler:say("We have a secret branch, called \'the stamps of the gods\'. They secretly supervise our members reliability and have an eye on certain groups of interest that want to gain influence over our guild.", cid)
	topic = 0
	
	elseif msgcontains(msg, 'member') and getPlayerStorageValue(cid,227) < 1 then
	npcHandler:say("We have high standards for our members. To rise in our guild is a difficult but rewarding task. Why do you ask? Are you interested in joining?", cid)
	topic = 1
	
	elseif msgcontains(msg, 'member') and getPlayerStorageValue(cid,227) > 0 then
	npcHandler:say("You are already a member, '.. getPlayerName(cid).. '.", cid)
	topic = 0
	...
	elseif msgcontains(msg, 'advancement') and getPlayerStorageValue(cid,STORAGE_POSTMAN_DOOR) >= getPlayerStorageValue(cid,249) then
	npcHandler:say("Sorry, but you are not yet ready for advancement.", cid)
	...
	npcHandler:say("From now on it shall be known that you are a grand postman. You are now a privileged member until the end of days. Most captains around the world have an agreement with our guild to transport our privileged members, like you, for less gold.", cid)
	...
	npcHandler:say("You have mastered all our current missions. There\'s nothing left to be done ... for now.", cid)
	topic = 0

	elseif msgcontains(msg, 'mission') and getPlayerStorageValue(cid,245) == 1 then
	npcHandler:say("You are not done with your current mission. Deliver that letter to king Markwin. Please report back when you are ready.", cid)
	topic = 0
	...
	npcHandler:say("You are not done with your current mission. Deliver those letters to Santa. Please report back when you are ready.", cid)
	topic = 0
	...
	npcHandler:say("Too bad, perhaps another time then.", cid)
	topic = 0
	...
	npcHandler:say("You are not done with your current mission. Search for the whereabout of Postofficer Waldo. Please report back when you are ready.", cid)
	topic = 0
	...
	elseif msgcontains(msg, 'yes') and topic == 19 and getPlayerItemCount(cid,2332) < 1 then
	npcHandler:say("Hm, no, you don\'t have it. Too bad, go and look for it.", cid)
	topic = 0
	
	elseif msgcontains(msg, '') and topic == 19 then
	npcHandler:say("Too bad, go and look for it.", cid)
	topic = 0
	...
	npcHandler:say("You are not done with your current mission. We still need the measurements of several postofficers. Please report back when you are ready.", cid)
	topic = 0
	...
	npcHandler:say("Too bad, perhaps you will try some other time then.", cid)
	topic = 0
	
	elseif msgcontains(msg, 'mission') and getPlayerStorageValue(cid,233) > 0 and getPlayerStorageValue(cid,233) < 10 then
	npcHandler:say("You are not done with your current mission. Make sure Hugo chief is tailoring our new uniforms. Please report back when you are ready.", cid)
	topic = 0
	...
	npcHandler:say("The queen has sent me the samples we needed. The next part is tricky. We need the uniforms to emanate some odor that dogs hate.The dog with the best \'taste\' in that field is Noodles, the dog of King Tibianus. Do you understand so far?", cid)
	topic = 15
	amount = math.random(1,3)
	
	elseif msgcontains(msg, 'yes') and topic == 15 then
	npcHandler:say("Good. Go there and find out what taste he dislikes most: moldy cheese, a piece of fur or a bananaskin. Tell him to SNIFF, then the object. Show him the object and ask \'Do you like that?\'. DONT let the guards know what you are doing.", cid)
	setPlayerStorageValue(cid,233,7)
	setPlayerStorageValue(cid,251,amount)
	topic = 0
	
	elseif msgcontains(msg, '') and topic == 15 then
	npcHandler:say("Too bad, perhaps you can try doing it some other time then.", cid)
	topic = 0
	...
	npcHandler:say("The mail with Talphion\'s instructions just arrived. I remember we asked Queen Eloise of Carlin for the perfect colours. Go there, ask her about the UNIFORMS and report back here.", cid)
	setPlayerStorageValue(cid,233,5)
	topic = 0
	
	elseif msgcontains(msg, 'mission') and getPlayerStorageValue(cid,231) == 1 then
	npcHandler:say("You are not done with your current mission. Deliver that present to Fibula. Please report back when you are ready.", cid)
	topic = 0
	...
	npcHandler:say("Ok. We need a new set of uniforms, and only the best will do for us. Please travel to Venore and negotiate with Hugo Chief a contract for new uniforms.", cid)
	...
	npcHandler:say("Do you bring a bone for our officers\' safety fund?", cid)
	topic = 12
	...
	npcHandler:say("Excellent! You have collected 1 bone. Just report about your mission again if you find  more.", cid)
	...
	elseif msgcontains(msg, 'yes') and topic == 12 and getPlayerItemCount(cid,2230) < 1 then
	npcHandler:say("You have no suitable bone with you. Too bad, but you surely will find some more.", cid)
	topic = 0
	...
	elseif msgcontains(msg, 'yes') and topic == 25 and getPlayerItemCount(cid,2230) < 20 then
	npcHandler:say("You have not enough bones with you. Too bad, but you surely will find some more.", cid)
	topic = 0
	
	elseif getPlayerStorageValue(cid,229) == 3 and msgcontains(msg, 'mission') then
	npcHandler:say("You truly got him? Quite impressive. You are a very prommising candidate! I think I have another mission for you. Are you interested?", cid)
	topic = 11	
	
	elseif getPlayerStorageValue(cid,229) > 0 and getPlayerStorageValue(cid,229) < 3 and msgcontains(msg, 'mission') then
	npcHandler:say("You are not done with your current mission. Find David Brassacres and hand him that bill. Report when you are ready.", cid)
	topic = 0
	
	elseif msgcontains(msg, 'mission') and getPlayerStorageValue(cid,228) == 1 then
	npcHandler:say("You are not done with your current mission. The mailbox is still not fixed. Report if you are ready.", cid)
	...
	npcHandler:say("You are not done with your current mission. Make sure all the passages are secure. Report if you are ready.", cid)
	topic = 0
	...
	npcHandler:say("I am glad to hear that. One of our mailboxes was reported to be jammed. It is located on the so called \'mountain\' on the isle Folda. Get a crowbar and fix the mailbox. Report about your mission when you have done so.", cid)
	...
	npcHandler:say("I thought so. The mail service is not for just anyone.", cid)
	topic = 0
	...
	npcHandler:say("I am sorry, but being illiterate disqualifies you from joining.", cid)
	topic = 0
