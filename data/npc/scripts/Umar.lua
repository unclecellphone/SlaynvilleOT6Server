...
function greetCallback(cid)
	...
	return false
	elseif getPlayerStorageValue(cid, 8169) <= 0 then
	return true
	else
...
if msgcontains(msg, "mission") or msgcontains(msg, "pass") or msgcontains(msg, "quest") then
	if getPlayerStorageValue(cid,8176) <= 0 then
	...
	talk_state = 801
	elseif getPlayerStorageValue(cid,8177) <= 99 then
	npcHandler:say("You're supposed to kill green djinns, and still here you are..", cid)
	talk_state = 0
	elseif getPlayerStorageValue(cid,8177) >= 100 and  getPlayerStorageValue(cid,8178) <= 0 then
	...
	npcHandler:story("You want another test?", cid)
	setPlayerStorageValue(cid,8178,1)
	talk_state = 701
	elseif getPlayerStorageValue(cid,8178) == 1 then
	npcHandler:say("You want another test?", cid)
	talk_state = 701
	elseif getPlayerStorageValue(cid,8180) == 1 and getPlayerStorageValue(cid,8179) <= 0 then
...
elseif talk_state == 801 and msgcontains(msg, "yes") then
	...
	npcHandler:story("Go in through the backdoor of the green djinn fortress, and assassinate 100 djinns. ...", cid)
	npcHandler:story("If you come back here alive again, we'll talk about if you're worthy.. now go, be strong!", cid)
	setPlayerStorageValue(cid,8170,1)
	setPlayerStorageValue(cid,8176,1)
	setPlayerStorageValue(cid,8177,0)
...
elseif talk_state == 701 and msgcontains(msg, "yes") then
	if getPlayerStorageValue(cid,8094) >= 1 and getPlayerStorageValue(cid,8178) == 1 then
		if getPlayerStorageValue(cid,8165) >= 1 then
			if getPlayerStorageValue(cid,8147) >= 1 then
			npcHandler:say("On the top floor of the green djinn fortress is a chest or a box, steal what's inside it and bring it to me! see you in a bit!", cid)
			setPlayerStorageValue(cid,8178,2)
			npcHandler:releaseFocus()
			npcHandler:resetNpc()	
			else
...
elseif talk_state == 601 and msgcontains(msg, "yes") then
	npcHandler:say("Oh it were empty, damn they knew abou.. herm.. well you are obviously worthy. ..", cid)
	npcHandler:say("You're welcome as a guest in our fortess, go and kill some green djinns when ever you want!", 5)
	setPlayerStorageValue(cid,8179,1)
	doAddQuestPoint(cid)
	talk_state = 0
