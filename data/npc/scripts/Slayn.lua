...
keywordHandler:addKeyword({'job'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "job? i just live for the gods of tibia."})
keywordHandler:addKeyword({'name'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "my name is Slayn.  my legend lives forever."})
...
keywordHandler:addKeyword({'life'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "in tibia there's whatever.  there are plants and people and monsters."})
keywordHandler:addKeyword({'plant'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "just walk around... you will see grass... trees... and bushes.... wow"})
keywordHandler:addKeyword({'people'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "i just know JJ, Martin, and JD. they all live in the main street to the north."})
...
keywordHandler:addKeyword({'muriel'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "muriel is a sorcerer. she is the keeper of arcane secrets that are known only to few mortals."})
...
keywordHandler:addKeyword({'time'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "Now, it is |TIME|. Ask Gorn for a watch, if you need one, you lazy piece of shit."})
...
function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'heal') then
		if getCreatureCondition(cid, CONDITION_FIRE) == true then
			npcHandler:say("you are on fire? please, just shut up.  come here.", cid)
			doRemoveCondition(cid, CONDITION_FIRE)
			doSendMagicEffect(getCreaturePosition(cid), 14)
		elseif getCreatureCondition(cid, CONDITION_POISON) == true then
			npcHandler:say("you are poisoned... idiot.", cid)
			doRemoveCondition(cid, CONDITION_POISON)
			doSendMagicEffect(getCreaturePosition(cid), 13)
		elseif getCreatureHealth(cid) < 40 then
			npcHandler:say("you are looking really bad... just terrible.", cid)
			doCreatureAddHealth(cid, 40 - getCreatureHealth(cid))
			doSendMagicEffect(getCreaturePosition(cid), 12)
		else
			npcHandler:say("you aren\'t looking that bad. if you are looking for additional protection you should wear a fucking diaper.", cid)
		end
		return true
	end
end
