...
function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'spell') then
		if getPlayerVocation(cid) >= 5 and getPlayerVocation(cid) <= 8 then
			npcHandler:say("I can teach 'Enchant Staff' to sorcerers, 'Challenge' to knights, 'Wild Growth' to druids, and 'Power Bolt' to paladins.", cid)
		else
			npcHandler:say("I am sorry, but you are not promoted yet.", cid)
		end
			talk_state = 0
	...
	elseif msgcontains(msg, 'enchant staff') or msgcontains(msg, 'Enchant Staff') then
		...
		if isInArray(spellvoc, getPlayerVocation(cid)) then
			...
			npcHandler:say("I am sorry but this spell is only for master sorcerers", cid)
			talk_state = 0
		end
	...
	elseif msgcontains(msg, 'challenge') or msgcontains(msg, 'Challenge') then
		...
		if isInArray(spellvoc, getPlayerVocation(cid)) then
			...
			npcHandler:say("I am sorry but this spell is only for elite knights", cid)
			talk_state = 0
		end
	...
	elseif msgcontains(msg, 'power bolt') or msgcontains(msg, 'Power bolt') then
		...
		if isInArray(spellvoc, getPlayerVocation(cid)) then
			...
			npcHandler:say("I am sorry but this spell is only for royal paladins", cid)
			talk_state = 0
		end
-- end of spells
...
-- teleport
	...
	elseif msgcontains(msg, 'wisdom') or msgcontains(msg, 'solitude') then
		npcHandler:say("I would provide you with the wisdom of solitude... But I fear our world has no longer the power...", cid)
		talk_state = 1394


	elseif talk_state == 1394 and msgcontains(msg, '') then
		npcHandler:say("Huh? What were wetalking about?", cid)
		talk_state = 0	
	
	elseif msgcontains(msg, 'phoenix') then
		npcHandler:say("The spark of the phoenix was once given by the dwarven priests of earth and fire in Kazordoon.", cid)
		talk_state = 0
	
	elseif msgcontains(msg, 'embrace') then
		npcHandler:say("The druids north of Carlin can't provide you with the embrace of tibia... anymore", cid)
		talk_state = 0
