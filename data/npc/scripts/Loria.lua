...
function creatureSayCallback(cid, type, msg) msg = string.lower(msg)
	...
	elseif msgcontains(msg, 'find person') or msgcontains(msg, 'Find person') then
		npcHandler:say("If you search someone, this spell will give you an idea of the direction you must head. You will be able to see, whether he is below or above you.", cid)	
	elseif msgcontains(msg, 'light healing') or msgcontains(msg, 'Light healing') then
		npcHandler:say("The paths to the next temple are long. Even in Tibia. So learn this spell, and be able to heal yourself during your travels. This spell will only cure small wounds, but it is pretty helpful.", cid)	
	elseif msgcontains(msg, 'light magic missile') or msgcontains(msg, 'Light magic missile') then
		npcHandler:say("You can activate this spell by pointing your index finger in the direction of your enemy, conjure the power of your rune and shoot the magic missiles in your enemy's body.", cid)	
	elseif msgcontains(msg, 'antidote') or msgcontains(msg, 'Antidote') then
		npcHandler:say("This spell sucks the venom out of your veins, that some enemy might have injected.", cid)	
	elseif msgcontains(msg, 'intense healing') or msgcontains(msg, 'Intense healing') then
		npcHandler:say("This spell will cure more wounds or greater ones at once. This is of course more 'mana intensive', but everybody will sooner or later get in a situation where mana is nothing - compared to life.", cid)	
	elseif msgcontains(msg, 'poison field') or msgcontains(msg, 'Poison field') then
		npcHandler:say("This spell will create a single field of poisonous gas. Cast it on a creature you were not able to arrange a peace treaty with. If it has no antidote, watch what could happen if you forget yours.", cid)	
	elseif msgcontains(msg, 'great light') or msgcontains(msg, 'Great light') then
		npcHandler:say("This spell will illuminate your whole screen and last longer than 'light'. Use it in deep dungeons, 'cause behind every corner there could be your last enemy ... and you might just walk into him.", cid)	
	elseif msgcontains(msg, 'fire field') or msgcontains(msg, 'Fire field') then
		npcHandler:say("This spell acts similar to the 'poison field' spell, except that you create fire instead of poisonous gas. Don't enter it yourself or you will realize why it is said that 'fire eats everything'.", cid)	
	elseif msgcontains(msg, 'heavy magic missile') or msgcontains(msg, 'Heavy magic missile') then
		npcHandler:say("Remember the spell where you only got to wave your hand? Well, wave it twice and shoot a heavy magic missile at your enemy. This spell will create a rune with five charges.", cid)	
	elseif msgcontains(msg, 'magic shield') or msgcontains(msg, 'Magic shield') then
		npcHandler:say("Well, mages are more bookworms than sportsmen, and as such often neglect their physical fitness. In ancient tomes lies the power to use mana as an equivalent to life. So use this spell to survive.", cid)		
	elseif msgcontains(msg, 'energy field') or msgcontains(msg, 'Energy field') then
		npcHandler:say("This one will create a field of energy. Everyone stepping in will be struck from lightning. This field will not last as long as poison or fire fields, but it is more deadly.", cid)	
	elseif msgcontains(msg, 'destroy field') or msgcontains(msg, 'Destroy field') then
		npcHandler:say("Trapped again between fire, poison and energy fields? This spell will give you the ability to destruct the fields, so you can pass safely.", cid)	
	elseif msgcontains(msg, 'fire wave') or msgcontains(msg, 'Fire wave') then
		npcHandler:say("Turn to you opponent and release the forces of nature with a whisper of your voice. A triangle of fire will burn all persons in your view, so take care, in which direction you look!", cid)	
	elseif msgcontains(msg, 'ultimate healing') or msgcontains(msg, 'Ultimate healing') then
		npcHandler:say("This spell is able to cure almost every injury at a higher cost than the other healing spells.", cid)	
	elseif msgcontains(msg, 'great fireball') or msgcontains(msg, 'Great fireball') then
		npcHandler:say("Imagine scaling the normal fireball by two and raising the fire temperature.", cid)	
	elseif msgcontains(msg, 'firebomb') or msgcontains(msg, 'fire bomb') then
		npcHandler:say("With a snip of your finger you can cover the floor with a burning carpet that keeps on burning for a while.", cid)		
	elseif msgcontains(msg, 'creature illusion') or msgcontains(msg, 'Creature illusion') then
		npcHandler:say("A good one to scare childs. You can change your appearance to any monster. You can be as handsome as a ghoul!", cid)	
	elseif msgcontains(msg, 'explosion') or msgcontains(msg, 'Explosion') then
		npcHandler:say("A strong blast of fire wounds the opponent you point at, and the adjacent squares.", cid)	
	elseif msgcontains(msg, 'great energy beam') or msgcontains(msg, 'Great energy beam') then
		npcHandler:say("A lightning bolt strikes the point you look at.", cid)
	elseif msgcontains(msg, 'energy beam') or msgcontains(msg, 'Energy beam') then
		npcHandler:say("A ray of energy will strike everyone in your current direction", cid)	
	elseif msgcontains(msg, 'invisible') or msgcontains(msg, 'Invisible') then
		npcHandler:say("This spell drains the colors out of you body, making yourself invisible for an hour or two.", cid)	
	elseif msgcontains(msg, 'summon creature') or msgcontains(msg, 'Summon creature') then
		npcHandler:say("This one gives you the ability to summon monsters that aid you in your battles.", cid)	
	elseif msgcontains(msg, 'energy wave') or msgcontains(msg, 'Energy wave') then
		npcHandler:say("Shoots a triangular bundle of lightning bolts in the direction you look.", cid)	
	elseif msgcontains(msg, 'sudden death') or msgcontains(msg, 'Sudden death') then
		npcHandler:say("The best spell for deciding a battle within seconds. The spell tries to interrupt the opponents heart beat, leading to his instant death in most cases.", cid)	
	elseif msgcontains(msg, 'light') or msgcontains(msg, 'Light') then
		npcHandler:say("A ray of light will emerge from your flat hand to illuminate your environment.", cid)	
	elseif msgcontains(msg, 'fireball') or msgcontains(msg, 'Fireball') then
		npcHandler:say("A perfect symbiosis of fire and wind. More is not to be said about this spell. Use this fireball as a warning or as your defence, but don't burn your fingers.", cid)	
	elseif msgcontains(msg, 'formula') or msgcontains(msg, 'Formula') then
		npcHandler:say("Which is the spell, you need the formula to?", cid)
		talk_state = 1	
