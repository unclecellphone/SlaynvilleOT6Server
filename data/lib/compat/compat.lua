...
function doRelocateForce(fromPos, toPos)
	if fromPos == toPos then	
		return false
	end
...
function getPlayerInstantSpellInfo(cid, spellId)
	local player = Player(cid)
	if not player then
		print("player cid bad, returns null " .. cid)
		return false
	end

	local spell = Spell(spellId)
	if not spell  then
		print("player spell bad, returns null " .. spellId)
		return false
	else
		if not player:canCast(spell) then
			return false
		end
	end
	return spell
