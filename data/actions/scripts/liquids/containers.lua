...
setCombatParam(lifeFluidCombat, COMBAT_PARAM_AGGRESSIVE, 0)
...
function onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	if(itemEx.uid == cid.uid) then -- Player is using on himself
		if(item.type == FLUID_NONE) then
			doPlayerSendCancel(cid, "It is empty.")
			return true
		end

		if(getCreatureCondition(cid, CONDITION_EXHAUST_POTION) ) then
			doPlayerSendDefaultCancel(cid, RETURNVALUE_YOUAREEXHAUSTED)
			return true
		end

		if(item.type == FLUID_MANA) then
			if not doPlayerAddMana(cid, math.random(38, 62)) then
				return false
			end
			doCreatureSay(cid, "Aaaah...", TALKTYPE_SAY)
			doSendMagicEffect(toPosition, CONST_ME_MAGIC_BLUE)
		elseif(item.type == FLUID_LIFE) then
			if not doCombat(cid, lifeFluidCombat, numberToVariant(cid.uid)) then
				return false
			end
			doCreatureSay(cid, "Aaaah...", TALKTYPE_SAY)
			doSendMagicEffect(toPosition, CONST_ME_MAGIC_BLUE)
	...
	if item.type == FLUID_BLOOD then
		local thing = getThingPos(toPosition)
		if thing then
			if thing.uid and thing.uid == POI_ENTRANCE_UID then
				doSendMagicEffect(toPosition, CONST_ME_MAGIC_BLUE)
				local newpos = {x=toPosition.x, y=toPosition.y, z=toPosition.z-1}
				doRelocate(cid, newPos)
				return true
			end
		end
	end
