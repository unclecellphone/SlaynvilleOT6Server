                                   ...
                                   Item* item, uint32_t count, Item** _moveItem, uint32_t flags /*= 0*/, Creature* actor/* = nullptr*/, Item* tradeItem/* = nullptr*/, const Position* fromPos /*= nullptr*/, const Position* toPos/*= nullptr*/)
{
	...
	if (!toItem || toItem->getID() != 1777)
	{
		//lets fucking go you're fucking toast bud
	...
	ReturnValue ret = fromCylinder->queryRemove(*item, m, flags, actor);
	if (ret != RETURNVALUE_NOERROR) {
		return ret;
	}
...
Item* Game::transformItem(Item* item, uint16_t newId, int32_t newCount /*= -1*/)
{
	try
	{
		...
		Cylinder* cylinder2 = item->getParent();
	...
	{
		std::cout << "some sort of fuckery is afoot no doubt" << std::endl;
		return nullptr;
	}
...
bool Game::playerBroadcastMessage(Player* player, const std::string& text) const
{
	...
	//this is unused, handled by scripts
	std::cout << "> " << player->getName() << " broadcasted: \"" << text << "\"." << std::endl;


	//for (const auto& it : players) {
	//	it.second->sendCreatureSay(player, TALKTYPE_SAY, text, &player->getPosition());
	//	if (it.second == player) {
	//		g_events->eventCreatureOnHear(it.second, player, text, TALKTYPE_SAY);
	//	}
	//}

	for (const auto& it : players) {
		it.second->sendPrivateMessage(player, TALKTYPE_SAY, text);
	}
...
void Game::playerChangeOutfit(uint32_t playerId, Outfit_t outfit)
{
	...
	if (player->canWear(outfit.lookType, outfit.lookAddons)) {
		...
		if (player->getCreatureCondition(CONDITION_OUTFIT)) {
			return;
		}
...
{
	if (player->getCreatureCondition(CONDITION_YELLTICKS)) {
	...
	if (!spectatorsPtr || spectatorsPtr->empty()) {
		...
		if (type != TALKTYPE_YELL) { //&& type != TALKTYPE_MONSTER_YELL) {
			...
			map.getSpectators(spectators, *pos, true, false,
						(Map::maxClientViewportX * 4) + 2, (Map::maxClientViewportX * 4) + 2,
						(Map::maxClientViewportY * 4) + 2, (Map::maxClientViewportY * 4) + 2);
		}
	} else {
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (damage.primary.value > 0) {
		...
		SpectatorVec spectators;
		if (targetPlayer && target->getCreatureCondition(CONDITION_MANASHIELD) && damage.primary.type != COMBAT_UNDEFINEDDAMAGE) {
...
void Game::playerLeaveParty(uint32_t playerId)
{
	...
	Party* party = player->getParty();
	if (!party || player->getCreatureCondition(CONDITION_INFIGHT)) {
		return;
	}
...
void Game::playerEnableSharedPartyExperience(uint32_t playerId, bool sharedExpActive)
{
	...
	Party* party = player->getParty();
	if (!party || (player->getCreatureCondition(CONDITION_INFIGHT) && player->getZone() != ZONE_PROTECTION)) {
		return;
	}
