	...
	if ((node = monsterNode.child("flags"))) {
		for (auto flagNode : node.children()) {
			...
			if (strcasecmp(attrName, "summonable") == 0) {
				mType->info.isSummonable = mType->info.manaCost > 0;
				...
				mType->info.isConvinceable = mType->info.manaCost > 0;
	...
	if ((node = monsterNode.child("defenses"))) {
		...
		for (auto defenseNode : node.children()) {
			...
			if (deserializeSpell(defenseNode, sb, monsterName)) {
				...
				//	std::cout << "[Warning - Monsters::loadMonster] Loaded spell. " << sb.chance << std::endl;
				}
				else
				{
					std::cout << "[Warning - Monsters::loadMonster] Cant load1 spell. " << file << std::endl;
				}
			} else {
				//std::cout << "[Warning - Monsters::loadMonster] Cant load2 spell. " << file << std::endl;
			}
		}
	}
