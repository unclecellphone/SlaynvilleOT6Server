...
//in fight ticks NEVER should be called with pzlock = true
//because the only thing that gives pz lock is killing
void Player::addInFightTicks(bool pzlock /*= false*/)
{
	...
	// hacked - you can be in a fight for 1 minute - but not PZ locked 
	...
	Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_INFIGHT, 60000, 0);
	addCondition(condition);
}

void Player::addPzLock()
{
	if (hasFlag(PlayerFlag_NotGainInFight)) {
		return;
	}
	...
	Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_INFIGHT, 60000 * 15, 0);
	addCondition(condition);
}
...
void Player::doAttacking(uint32_t)
{
	...
	if (getCreatureCondition(CONDITION_PACIFIED)) {
		return;
	}
...
bool Player::onKilledCreature(Creature* target, bool lastHit/* = true*/)
{
	...
	if (!hasFlag(PlayerFlag_NotGainInFight) && !isPartner(targetPlayer)) {
		if (!Combat::isInPvpZone(this, targetPlayer) && hasAttacked(targetPlayer) && !targetPlayer->hasAttacked(this) && !isGuildMate(targetPlayer) && targetPlayer != this) {
			...
			if (lastHit && getCreatureCondition(CONDITION_INFIGHT)) {
				pzLocked = true;
				//hack - do not use whiteskull time,  which is set to 0 because fuck that noise
				//use 15 minute hardcoded timer instead
				...
				Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_INFIGHT, 15 * 60 * 1000, 0);
				addCondition(condition);
			}
		}
	}
...
void Player::checkSkullTicks(int64_t ticks)
{
	...
	//if ((skull == SKULL_RED || skull == SKULL_BLACK) && skullTicks < 1 && !getCreatureCondition(CONDITION_INFIGHT)) {
	if (skull == SKULL_RED && skullTicks < 1 && !getCreatureCondition(CONDITION_INFIGHT)) {
		setSkull(SKULL_NONE);
	}
}
...
bool Player::toggleMount(bool mount)
{
	...
	if (mount) {
		...
		if (getCreatureCondition(CONDITION_OUTFIT)) {
