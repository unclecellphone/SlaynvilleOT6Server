...
bool Spell::playerSpellCheck(Player* player) const
{
	...
	if ((aggressive || pzLock) && player->getCreatureCondition(CONDITION_PACIFIED)) {
	...
	if (player->getCreatureCondition(CONDITION_SPELLGROUPCOOLDOWN, group) || player->getCreatureCondition(CONDITION_SPELLCOOLDOWN, spellId) || (secondaryGroup != SPELLGROUP_NONE && player->getCreatureCondition(CONDITION_SPELLGROUPCOOLDOWN, secondaryGroup))) {
		player->sendCancelMessage(RETURNVALUE_YOUAREEXHAUSTED);
...
bool Spell::playerRuneSpellCheck(Player* player, const Position& toPos)
{
	...
	// we remvoed the "range" check here because what the fuck is that?
	// the range is: i clicked on it! that's the range.
...
void Spell::postCastSpell(Player* player, bool finishedCast /*= true*/, bool payCost /*= true*/) const
{
	...
	//no soul
	if (payCost) {
		Spell::postCastSpell(player, getManaCost(player), 0);// getSoulCost());
	}
}
...
bool InstantSpell::canCast(const Player* player) const
{
	...
	if (isLearnable()) {
		...
		try
		{
			...
			const auto c = vocSpellMap.find(player->getVocationId());

			if (c != vocSpellMap.end()) {
				return true;
			}
		}
...
ReturnValue RuneSpell::canExecuteAction(const Player* player, const Position& toPos)
{
	...
	return RETURNVALUE_NOERROR;
			...
			//(&RuneSpell::executeUseLate, runeAction, player->getID(), item->getID(), fromPos, Position&, targetID, toPos, isHotkey);
bool RuneSpell::executeUseLate(uint32_t playerCid,  const Position& fromPos, uint32_t itemID, uint32_t targetCid, const Position& toPosition, bool isHotkey)
{
	/*if (!scripted) {
		return false;
	}*/
	
	Thing* target = g_game.getCreatureByID(targetCid);
	Player* player = g_game.getPlayerByID(playerCid);
	Item* item = Actions::getRune(itemID);
	if (!item)
	{
	...
	{
		std::cout << "[RuneSpell::executeUseLate] ERROR GETTING ITEM ITS LIKELY DEAD" << std::endl;
		//SHOULD BE "YOU NEED A MAGIC ITEM FOR THIS SPELL"  OR SOMETHING
		player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
		g_game.addMagicEffect(player->getPosition(), CONST_ME_POFF);
		return false;
	}
	...
	if (player)
	{
		if (needTarget) {
			...
			if (target == nullptr) {
				//this should literally never be used, at least in pvp, maybe from battlelist?
		...
		// new rune spell use case
		// we check the location of the player NOW
		// rather than before the delay, because otherwise we'd be checking
		// where the player was, rather than where they are
		auto p = toPosition;
		if (target)
			p = target->getPosition();
		if (!playerRuneSpellCheck(player, p)) {
			player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
			g_game.addMagicEffect(player->getPosition(), CONST_ME_POFF);
			return false;
		}
		...
		if (hasCharges && item && g_config.getBoolean(ConfigManager::REMOVE_RUNE_CHARGES)) {
			...
			if (newCount == 0) {
				if (Actions::deregisterRuneId(itemID))
				{
					//std::cout << "[Spells] successfully deregistered rune" << std::endl;
				}
				else
				{
					//std::cout << "[Spells] FAILED to deregister rune" << std::endl;
				}
			}
