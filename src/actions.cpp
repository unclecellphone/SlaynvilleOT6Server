...
void Actions::clearRuneMap(RuneUseMap& map, bool fromLua)
{
	for (auto it = map.begin(); it != map.end(); ) {
			it = map.erase(it);
			++it;
...
void Actions::clear(bool fromLua)
{
	...
	clearRuneMap(runeItemMap, fromLua);
...
int32_t Actions::registerRune(Item* rune)
{
	if (rune == nullptr)
		return false;
	if (rune->getUniqueRuneId() >= 0)
		return rune->getUniqueRuneId();

	int key = runeItemMap.size();
	int originalCount = key;
	int overflow = 1000;
	while (runeItemMap.contains(key) && key < originalCount + overflow)
		key++;

	if (runeItemMap.contains(key))
		return -1;
	if(key < 0)
		return false;

	//insert rune
	runeItemMap.emplace(key, rune);
	rune->setUniqueRuneId(key);
	return key >= 0 ? key : -1;
}

bool Actions::deregisterRuneId(int32_t id)
{
	if (id < 0)
		return false;
	bool hasRune = runeItemMap.contains(id);
	if(hasRune)
		runeItemMap.erase(id);
	return hasRune;
}

Item* Actions::getRune(int32_t id)
{
	bool hasRune = runeItemMap.contains(id);
	if (hasRune)
		return runeItemMap[id];
	else
		return nullptr;
}
                        ...
                        uint8_t toStackPos, Item* item, bool isHotkey, Creature* creature/* = nullptr*/)
{
	...
	if (auto runeAction = dynamic_cast<RuneSpell*>(action))
	{
		//a runeish thing
		Tile* toTile = g_game.map.getTile(toPos);
		if (toTile) {
			const Creature* visibleCreature = toTile->getBottomVisibleCreature(player);
			int32_t runeID = Actions::registerRune(item);
			if (runeAction->getNeedTarget() && visibleCreature)
			{
			...
			{
				//the creature was not a thing, nice! (what?)
		...
		{
			//you clicked on a non-tile? not sure what that even means, im impressed
			player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
			g_game.addMagicEffect(player->getPosition(), CONST_ME_POFF);
			return false;
		}
