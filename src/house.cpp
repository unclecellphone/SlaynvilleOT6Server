...
void House::updateDoorDescription() const
{
	...
	for (const auto& it : doorSet) {
		it->setSpecialDescription(fmt::format("It belongs to house '{:s}'. {:s} owns this house.{:s}", houseName, (owner != 0) ? "Somebody" : "Nobody", g_config.getBoolean(ConfigManager::HOUSE_DOOR_SHOW_PRICE) && (housePrice != -1) && (owner == 0) ? fmt::format(" It costs {:d} gold coins.", (houseTiles.size() * housePrice)) : ""));
	}
}
	...
	for (const auto& it : houseMap) {
		...
		if (player.getBankBalance() >= rent) {
			...
			if (house->getPayRentWarnings() < 7) {
				...
				g_game.internalAddItem(player.getDepotLocker(house->getTownId()), letter, INDEX_WHEREEVER, FLAG_NOLIMIT);
