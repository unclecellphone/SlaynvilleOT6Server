...
bool Monster::isTarget(const Creature* creature) const
{
	...
	//if i am a player
	bool imPlayer = getPlayer();


	//technically summons can be wild if they
	//are the summon of a wild creature (demon spawns FE, etc)
	//this will set imPlayerSummon false if the master is not a player
	bool imPlayerSumm = isSummon() && getMaster() && getMaster()->getPlayer();


	//if targetlist creature is not a player
	bool targetCreatureIsAPlayer = creature->getPlayer();

	//technically summons can be wild if they
	//are the summon of a wild creature (demon spawns FE, etc)
	//this will set hasPlayerMaster false if the master is not a player
	bool targetCreatureIsAPlayerSummon = creature->isSummon() && creature->getMaster() && creature->getMaster()->getPlayer();



	if (!imPlayer && !imPlayerSumm)
		if(!targetCreatureIsAPlayer && !targetCreatureIsAPlayerSummon)
			return false;
...
void Monster::onThinkDefense(uint32_t interval)
{
	...
	for (const spellBlock_t& spellBlock : mType->info.defenseSpells) {
		...
		if ((spellBlock.chance >= static_cast<uint32_t>(uniform_random(1, 100)))) {
			minCombatValue = spellBlock.minCombatValue;
			maxCombatValue = spellBlock.maxCombatValue;
			if (spellBlock.spell && spellBlock.spell != nullptr)
			{
				auto me = getName();
			...
			{
				std::cout << "ERROR: SPELL WAS NULL: " << std::endl;
			}
		}
	}
