...
{
	if (!getCreatureCondition(CONDITION_DRUNK)) {
		return;
	}
...
void Creature::goToFollowCreature()
{
	if (followCreature) {
		...
		bool isPlayer = getPlayer();
		bool isPlayerSumm = isSummon();
		if (isPlayerSumm)
			isPlayerSumm = getMaster() && getMaster()->getPlayer();

		//if we are a free creature, no master
		//and we are not a player
		//then we are just a random monster in the wild
		if (!isPlayer && !isPlayerSumm)
		{
			//in this case we should not be following or attacking
			//other free creatures out in the wild.
			//this behavior can stem from rogue summons
			bool followIsPlayer = followCreature->getPlayer();
			bool followIsSummon = followCreature->isSummon();
			if (followIsSummon)
				followIsSummon = followCreature->getMaster() && followCreature->getMaster()->getPlayer();
			if (!followIsPlayer && !followIsSummon)
			{
				if (isSummon() && getMaster() == followCreature)
				{
					//im a monster summon and im following my master who is a monster too
				}
				else
				{
					std::cout << "Creature ditching follow.  We are likely an exSummon who had a target dismissed: " << getName() << std::endl;
...
{
	if (type == CONDITION_PARALYZE && getCreatureCondition(CONDITION_HASTE)) {
		removeCondition(CONDITION_HASTE);
	} else if (type == CONDITION_HASTE && getCreatureCondition(CONDITION_PARALYZE)) {
...
bool Creature::setMaster(Creature* newMaster, bool preserve) {
	if (!newMaster && !master) {
		return false;
	}
...
bool Creature::addCondition(Condition* condition, bool force/* = false*/)
{
	...
	if (!force && condition->getType() == CONDITION_HASTE && getCreatureCondition(CONDITION_PARALYZE)) {
...
bool Creature::getCreatureCondition(ConditionType_t type, uint32_t subId/* = 0*/) const
{
	if (isSuppress(type)) {
		return false;
	}
