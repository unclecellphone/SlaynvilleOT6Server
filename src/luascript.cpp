...
{
	// creature:setMaster(master, false)
	Creature* creature = getUserdata<Creature>(L, 1);
	if (!creature) {
		lua_pushnil(L);
		return 1;
	...
	pushBoolean(L, creature->setMaster(getCreature(L, 2), false));
...
{
	// creature:getCreatureCondition(conditionType[, subId = 0])
	Creature* creature = getUserdata<Creature>(L, 1);
	if (!creature) {
		lua_pushnil(L);
		return 1;
	...
	pushBoolean(L, creature->getCreatureCondition(conditionType, subId));
	return 1;
}
...
int LuaScriptInterface::luaItemTypeGetMinReqLevel(lua_State* L)
{
	...
	const ItemType* itemType = getUserdata<const ItemType>(L, 1);
	if (itemType) {
		lua_pushinteger(L, 1); // itemType->minReqLevel);
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaItemTypeGetRequiredLevel(lua_State* L)
{
	...
	const ItemType* itemType = getUserdata<const ItemType>(L, 1);
	if (itemType) {
		lua_pushnumber(L, 1);// itemType->minReqLevel);
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaWeaponRegister(lua_State* L)
{
	...
	if (auto* weapon = *weaponPtr) {
		...
		//	it.minReqLevel = 1;// weapon->getReqLevel();
		//	it.minReqMagicLevel = 1; //weapon->getReqMagLv();
		//}
...
int LuaScriptInterface::luaWeaponLevel(lua_State* L)
{
	...
	Weapon* weapon = getUserdata<Weapon>(L, 1);
	if (weapon) {
		weapon->setRequiredLevel(1); // getNumber<uint32_t>(L, 2));
