...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result)
{
	...
	if ((result = db.storeQuery(fmt::format("SELECT `player_id`, `name` FROM `player_spells` WHERE `player_id` = {:d}", player->getGUID())))) {
		do {
			auto spellName = result->getString("name");

			//fix "invisible " wtf bug
			auto invisibleMissspellingOne = "invisible ";
			auto invisibleMissspellingTwo = "invisible";
			if (strcasecmp(spellName.c_str(), invisibleMissspellingOne) == 0) {
				spellName = "Invisibility";
			}
			if (strcasecmp(spellName.c_str(), invisibleMissspellingTwo) == 0) {
				spellName = "Invisibility";
			}
			player->learnedInstantSpellList.emplace_front(spellName);
		} while (result->next());
	}
